...
function create3DHearts() {
    ...
    for (let i = 0; i < 100; i++) { // Tăng số lượng trái tim
        const material = new THREE.MeshPhongMaterial({
...
function createFloatingHearts() {
    const heartCount = 150; // Nhiều trái tim bay
...
function setupEventListeners() {
    ...
    document.getElementById('reset').addEventListener('click', () => {
        ...
        if (auroraSpeedSlider) auroraSpeedSlider.value = 1.0;
        if (auroraSpeedValue) auroraSpeedValue.textContent = '1.0';
    ...
    function resetAuroraSettings() {
        setAuroraBrightness(1.0);
        setAuroraColorMode('rainbow');
        setAuroraColorIntensity(1.0);
        ...
        if (heartUpload) {
            heartUpload.onchange = (event) => {
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    const countChoice = prompt('Số lượng ảnh muốn thêm (1-20, mặc định 10):', '10');
                    const count = Math.max(1, Math.min(20, parseInt(countChoice) || 10));

                    const sizeChoice = prompt('Chọn kích thước ảnh (1-5, mặc định 3):', '3');
                    const size = Math.max(1, Math.min(5, parseInt(sizeChoice) || 3));
...
function createFlowerRain() {
    ...
    for (let i = 0; i < flowerCount; i++) {
        ...
        // Cánh hoa
        const petalCount = 5;
        const petalColors = [0xff69b4, 0xff1493, 0xffc0cb, 0xffd1dc, 0xff91a4];

        for (let j = 0; j < petalCount; j++) {
            const petalGeometry = new THREE.SphereGeometry(0.25, 8, 6);
            petalGeometry.scale(1, 0.3, 0.8);
    ...
    console.log(`Đã tạo ${flowerCount} bông hoa`);
}
...
function updateFlowerRain(elapsedTime) {
    ...
    rainSystem.flowers.forEach((flower, i) => {
        ...
        // Hiệu ứng thay đổi kích thước nhẹ
